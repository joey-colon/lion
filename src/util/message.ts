...
const linkPrefix = 'https://discord.com/channels';
const arrows = ['⬅️', '➡️'];
const cancelEmote = '❎';

function getBotReportChannel(guild: Guild): TextChannel | undefined {
  ...
  for (const channel of channels.cache) {
    ...
    if (channelObject.name === Constants.Channels.Admin.BotLogs) {
      return channelObject as TextChannel;
    }
  }
}

function sendConstructedReport(channel: TextChannel, report: string, options?: {}) {
  if (!options) {
    channel.send(report);
  } else {
    channel.send(report, options);
  }
}
...
export const MessageService = {
  ...
  sendBotReportOnMessage(message: IMessage): void {
    ...
    report += `${linkPrefix}/${message.guild?.id}/${message.channel.id}/${message.id}`;
    sendConstructedReport(message.channel as TextChannel, report, { files: message.attachments.map((e) => e.url) });
  },
  ...
  async sendReactiveMessage(
    ...
    await msg.react(cancelEmote); // Makes cancel available on all reactions (We could also make it an option in the future)
    ...
    const collector = msg.createReactionCollector(
      (reaction: MessageReaction, user: User) =>
        (embedData.emojiData.some((reactionKey) => reactionKey.emoji === reaction.emoji.name) ||
          reaction.emoji.name === cancelEmote) &&
    ...
    collector.on('collect', async (reaction: MessageReaction) => {
      ...
      if (reaction.emoji.name === cancelEmote) {
        collector.stop();
        return;
      }
  ...
  async attemptDMUser(message: IMessage, content: string | MessageEmbed) {
    try {
      ...
      await message.channel.send(content).catch((e) => winston.error(e));
    }
  },
  ...
  async sendPagedEmbed(message: IMessage, _pages: MessageEmbed[]): Promise<IMessage> {
    ...
    await Promise.all(arrows.map((a) => msg.react(a)));

    const collector = msg.createReactionCollector(
      (reaction: MessageReaction, user: User) =>
        arrows.includes(reaction.emoji.name) && user.id !== msg.author.id, // Only run if its not the bot putting reacts
      {
        time: 1000 * 60 * 10,
